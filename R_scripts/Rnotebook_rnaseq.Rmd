...
countData <- read.csv('BEAR_adult_SEGvNEG.csv')
head(countData)
```
...
```{r}
colData <- read.csv('bear_metadata_adult.csv')
head(colData)
...
write.csv(res, file="rnaseq_adult_newgenome_deseq2_gender_group.csv")
```
...
pcaData <- plotPCA(rld, intgroup=c("group", "gender"), returnData=TRUE)
